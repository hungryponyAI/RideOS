...
interface Props {
  thumbnail: number[];   // elevation values
  totalDistM: number;
  startM: number;
  endM: number;
...
export function RouteTrimSlider({ thumbnail, totalDistM, startM, endM, onChange }: Props) {
  ...
  const onPointerMove = useCallback((e: React.PointerEvent) => {
    ...
    const rawM = xToDist(svgX);
    if (dragRef.current.handle === "start") {
      const newStart = snap(Math.min(rawM, endM - MIN_SPAN_M));
      onChange(newStart, endM);
    } else {
      const newEnd = snap(Math.max(rawM, startM + MIN_SPAN_M));
      onChange(startM, newEnd);
    }
  ...
  const xStart = distToX(startM);
  const xEnd = distToX(endM);
    ...
    <div className="flex flex-col gap-1.5 select-none">
      ...
      >
        ...
        <clipPath id="trim-clip">
          <rect x={xStart} y={0} width={xEnd - xStart} height={SVG_H} />
        </clipPath>
        <path d={pathD} fill="#FFF200" stroke="#000" strokeWidth="1.5"
          vectorEffect="non-scaling-stroke" clipPath="url(#trim-clip)" />
        {/* Left dimmed overlay */}
        ...
        {/* Left handle */}
        <rect
          x={xStart - 3} y={0} width={6} height={SVG_H}
          fill="#FFF200" className="cursor-ew-resize"
          onPointerDown={e => onPointerDown(e, "start")}
        />
        {/* Right handle */}
        <rect
          x={xEnd - 3} y={0} width={6} height={SVG_H}
          fill="#FFF200" className="cursor-ew-resize"
      ...
      <div className="flex justify-between px-0.5">
        <span className="text-[9px] font-condensed font-bold tabular-nums text-[#FFF200]">
          {(startM / 1000).toFixed(1)} km
        </span>
        <span className="text-[9px] font-condensed font-bold tabular-nums text-[var(--text-muted)]">
          {((endM - startM) / 1000).toFixed(1)} km ausgewählt
        </span>
        <span className="text-[9px] font-condensed font-bold tabular-nums text-[#FFF200]">
          {(endM / 1000).toFixed(1)} km
        </span>
      </div>
    </div>
  );
