...
import type { RouteLibraryEntry } from "../types/route";
...
function estimateTimeS(
  distanceKm: number, elevationGainM: number, ftpW: number, weightKg: number, heightCm: number,
): number {
  const rho = 1.225, g = 9.81, crr = 0.004;
...
export const RouteCardExpanded = memo(function RouteCardExpanded({
  ...
  const handleToggleTrim = useCallback(() => {
    if (!trimEnabled) {
      setTrimStart(0);
      setTrimEnd(totalDistM);
    }
    setTrimEnabled(t => !t);
  }, [trimEnabled, totalDistM]);

  const handleStart = useCallback(() => {
    const finalConfig: RideConfig = {
      ...config,
  ...
  const estTime = estimateTimeS(
    route.distance_km, route.elevation_gain_m,
    athleteSettings.ftp_w, athleteSettings.weight_kg, athleteSettings.height_cm,
  );
  ...
  return (
    <div className="flex flex-col bg-[var(--surface)] border border-[#FFF200] overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border)]">
        <div className="flex-1 min-w-0">
          {editing ? (
            <input
              ...
              onKeyDown={e => {
                ...
                if (e.key === "Escape") { setEditing(false); setNameVal(route.name); }
              }}
            ...
            <div className="flex items-center gap-1.5 min-w-0">
              {route.strava_id && (
                <svg width="10" height="10" viewBox="0 0 24 24" fill="#FC4C02" aria-hidden="true">
                  <path d="M15.387 17.944l-2.089-4.116h-3.065L15.387 24l5.15-10.172h-3.066m-7.008-5.599l2.836 5.598h4.172L10.463 0l-7 13.828h4.169" />
                </svg>
              )}
              <span
                className="text-[14px] font-condensed font-bold text-[var(--text)] truncate cursor-pointer"
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-[9px] font-condensed font-bold tracking-widest text-[var(--text-muted)] tabular-nums">
            ...
            {route.best_time_s !== null ? (
              <>
                <span className="text-[9px] font-condensed font-bold uppercase text-[#22C55E]">BEST</span>
                <span className="text-[10px] font-data font-bold tabular-nums text-[#22C55E]">{formatTime(route.best_time_s)}</span>
              </>
            ) : (
              <>
                <span className="text-[9px] font-condensed font-bold uppercase text-[var(--text-muted)]">EST</span>
                <span className="text-[10px] font-data font-bold tabular-nums text-[var(--text-muted)]">{formatTime(estTime)}</span>
              </>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className="w-6 h-6 flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--text)] cursor-pointer font-bold text-base leading-none"
          >
            ×
          </button>
        </div>
      ...
      {/* Elevation profile / trim slider area */}
      <div className="px-4 pt-3 pb-2 border-b border-[var(--border)]">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <button
            ...
            className={`text-[9px] font-condensed font-bold tracking-widest uppercase px-2 py-0.5 border transition-colors cursor-pointer ${
              trimEnabled
                ? "border-[#FFF200] text-[#FFF200]"
        ...
        {route.elevation_thumbnail.length >= 2 ? (
          trimEnabled ? (
            <RouteTrimSlider
              thumbnail={route.elevation_thumbnail}
            ...
            <div style={{ height: 72 }}>
              ...
              >
                {(() => {
                  ...
                  const minE = Math.min(...thumb);
                  const maxE = Math.max(...thumb);
                  const range = Math.max(maxE - minE, 1);
                  const pts = thumb.map((e, i) => {
                    const x = (i / (n - 1)) * 1000;
                    const y = 80 - ((e - minE) / range) * 72;
                    return `${x.toFixed(1)},${y.toFixed(1)}`;
                  });
                  const d = `M0,80 L${pts.join(" ")} L1000,80 Z`;
                  return (
                    <path d={d} fill="#FFF200" stroke="#000" strokeWidth="1.5" vectorEffect="non-scaling-stroke" />
                  );
                })()}
              </svg>
            </div>
      ...
      <div className="px-4 py-3 border-b border-[var(--border)]">
        <RideOptions
          config={config}
          totalDistM={totalDistM}
          hasStravaOrBestTime={hasStravaOrBestTime}
          onChange={setConfig}
        />
      </div>
      ...
      <div className="px-4 py-3 flex justify-end">
        <button
          type="button"
          onClick={handleStart}
          className="bg-[#FFF200] text-black font-condensed font-bold text-[13px] tracking-widest uppercase px-10 py-3 cursor-pointer hover:bg-white transition-colors duration-150"
        >
          STARTEN →
        </button>
      </div>
