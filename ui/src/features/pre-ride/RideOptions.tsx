    ...
    <button
      ...
      className={`flex flex-col items-start px-3 py-2 border transition-colors duration-100 cursor-pointer min-w-0 ${
        disabled ? "border-[var(--border)] opacity-30 cursor-not-allowed"
          : checked ? "border-[#FFF200] bg-[var(--surface)]"
          : "border-[var(--border)] bg-transparent hover:border-[var(--text-muted)]"
      }`}
    >
      <div className="flex items-center gap-2">
        <div className={`w-2.5 h-2.5 rounded-full border-2 shrink-0 ${checked && !disabled ? "border-[#FFF200] bg-[#FFF200]" : "border-[var(--text-muted)] bg-transparent"}`} />
        <span className="text-[10px] font-condensed font-bold tracking-widest uppercase text-[var(--text)]">{label}</span>
      </div>
      {sublabel && <span className="text-[9px] font-condensed text-[var(--text-muted)] tracking-wide ml-4.5 mt-0.5 leading-tight">{sublabel}</span>}
    </button>
  );
}
...
function LapStepper({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  return (
    <div className="flex flex-col items-start px-3 py-2 border border-[var(--border)] bg-transparent gap-1">
      <span className="text-[10px] font-condensed font-bold tracking-widest uppercase text-[var(--text)]">LAPS</span>
      <div className="flex items-center gap-2">
        <button type="button" onClick={() => onChange(Math.max(1, value - 1))}
          className="w-5 h-5 flex items-center justify-center border border-[var(--border)] text-[var(--text-muted)] hover:border-[#FFF200] hover:text-[var(--text)] cursor-pointer text-sm leading-none transition-colors">−</button>
        <span className="text-[13px] font-data font-bold tabular-nums text-[var(--text)] w-4 text-center">{value}</span>
...
export function RideOptions({ config, hasStravaOrBestTime, onChange }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <span className="text-[9px] font-condensed font-bold tracking-[0.2em] uppercase text-[var(--label-accent)]">OPTIONEN</span>
